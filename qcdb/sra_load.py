...
def query_json(email, samplemeta_id):
    Entrez.email=email
    srs = samplemeta_id.split("_")[0]
    srx = samplemeta_id.split("_")[1]
    handle = Entrez.esearch(db="SRA",term=srs)
    record = Entrez.read(handle)

query_json("august_guang@brown.edu","SRS643408_SRX612442")

### everything below is from bioflows... may or may not be useful.
